...
export function convertPixiContainerToSkia(container, skCanvas, canvasKit) {
...
function drawDisplayObject(displayObject, skCanvas, canvasKit) {
    skCanvas.save();
        ...
        if (obj instanceof PIXI.Graphics) {
            ...
            for (const data of obj.geometry.graphicsData ?? []) {
                ...
                if (shape instanceof PIXI.Ellipse) {
                    const rect = canvasKit.XYWHRect(shape.x - shape.width, shape.y - shape.height, shape.width * 2, shape.height * 2);
                    path.addOval(rect);
                }
            ...
            if (sprite.texture &&
                ...
                if (resource.source) {
                    const imageElement = resource.source;
                    ...
                    if (skImage) {
                        ...
                        const destRect = canvasKit.XYWHRect(anchorOffsetX, anchorOffsetY, sprite.width, sprite.height);
...
export function setupInteractiveHandlers(skCanvasElement) {
